// Seed script to populate courses to Neon database
// This script calls the API routes we created
  ...
  try {
    const response = await fetch(`${BASE_URL}/api/admin/courses/${courseSlug}`, {
      ...
      headers: {
        ...
        // Add admin auth token here if needed
      },
    })
...
async function main() {
  ...
  // Seed one at a time to avoid overwhelming the server
